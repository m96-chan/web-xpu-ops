...
def main() -> None:
    ...
    (args.out / "dit.manifest.json").write_text(json.dumps({
        ...
        "repo": REPO,
        # The terms, not only the address. `lylogummy/Anima-3.8B` declares no
        # licence of its own and defers to an upstream it does not name, which
        # is why tracing this took an hour (issue #190). Recording the answer
        # here means the next person reads it off the artifact.
        "license": "circlestone-labs-non-commercial-license",
        "licenseUrl": "https://huggingface.co/circlestone-labs/Anima/blob/main/LICENSE.md",
        "licenseChain": [
            ...
            "lylogummy/Anima-3.8B (no licence declared; defers upstream)",
        ],
        "commercialUse": False,
