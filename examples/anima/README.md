...
A prompt goes in and an image comes out, on the GPU, in 233 seconds at
1216x832 — 178 s of sampling, 26 s of conditioning and 29 s of decoding.
...
| commit | `e708c33` (2026-09-06); rows marked † were not re-measured there and date from `6a830ea` (2026-08-25) |

| | |
| --- | --- |
| conditioning (both prompts, CPU) | 26.2 s |
| sampling, 80 model calls | **177.5 s** — 2.22 s each |
| VAE decode at 1216x832 | **28.8 s** (27.3 s on a second run; 17.8 s when last recorded, and why it is slower now is not measured) |
...
The sampling row said 672 s — 8.40 s a call — from 2026-08-25 until issue #228;
`26c7020` (#178) retuned the kernels the same day the row was written and the
row was never re-run. The 3.8x is that tuning, not anything in this port.
