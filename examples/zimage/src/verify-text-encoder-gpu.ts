/**
 * The GPU encoder, against the same golden the CPU one is held to.
 *
 * Issue #166 stage 5. Compared against the model, not against the CPU port —
 * two ports that drifted the same way would agree with each other and with
 * nothing else.
 *
 * Issue #166 stage 3's notes: The weights are read straight out of the checkpoint's
...
import { qwen3EncodeGpu } from "./text-encoder-gpu.js";
...
async function main(): Promise<void> {
  ...
  if (!runner) {
    console.error("verify-text-encoder-gpu: no WebGPU adapter available.");
    process.exit(2);
  }
  const started = Date.now();
  const got = await qwen3EncodeGpu(runner.run, encoderKernels(), cfg, weights, ids);
  console.log(`  encoded in ${((Date.now() - started) / 1000).toFixed(1)}s (GPU)`);
  close();
