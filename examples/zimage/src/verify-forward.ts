/**
 * Runs the whole DiT forward on the shipped weights and compares it to the model.
 *
 * Issue #166 stage 2. `verify-real-block.ts` covers one layer; this covers the
...
import { type DitConfig, type DitTrace, ditForward } from "./dit.js";
...
function main(): void {
  ...
  console.log(`  forward in ${((Date.now() - ran) / 1000).toFixed(1)}s (CPU)`);
