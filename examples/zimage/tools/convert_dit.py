...
from models import LICENSE, LICENSE_URL, REPO, add_argument, resolve  # noqa: E402
...
def main() -> None:
    ...
    (args.out / "dit.manifest.json").write_text(json.dumps({
        ...
        # Where these weights came from and under what terms. Converting a model
        # does not relicense it, and an artifact that cannot name its source is
        # one nobody can decide anything about later -- see issue #190, which
        # exists because Anima's conversion could not.
        "repo": REPO,
        "license": LICENSE,
