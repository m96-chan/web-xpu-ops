 ...
 * `harness/wgsl.ts`.
 *
 * Two copies of `ResidentDevice` exist because the two run on opposite sides of
 * exactly the module boundary `webgpu` draws, not because anything here is
 * demo-specific. If they drift, a forward produces different numbers in the
 * browser than the verifier measures in Node — which is visible, since both are
 * held to the same golden.
 *
 * **It lives here because the drift the paragraph above worries about
 * happened.** `anima-web` and `zimage-web` each had a copy, and they were not
 * the same file: this one had the profiling half and the other did not, with a
 * comment saying a stub that reported nothing would be worse than its absence —
 * which is precisely the state this one reached and then fixed. Two demos with
 * two answers to the same question is one demo's answer being wrong.
 *
