...
describe("h3-audio-web / markup", () => {
  ...
  it("loads the bundle by a relative path, so a subdirectory still works", () => {
    ...
    // page can catch** — so the page renders its static markup, the dialog
    // "exists" and nothing opens it. Measured on Pages before it was fixed.
    expect(html).toMatch(/src="\.\/dist\/bundle\.js"/);
    expect(html).not.toMatch(/(src|href)="\/[^/]/);
  });

  it("says on the page that there is no prompt, rather than offering one", () => {
    // The decoder is 260 MB; the transformer that would write its latent and
    // the encoder that would read a prompt are 27 GB between them and are not
    // here. A page with a prompt box that did nothing would be worse than one
