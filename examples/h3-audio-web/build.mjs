#!/usr/bin/env node
/**
 * Bundles the MiniMax-H3 audio decoder demo into `dist/bundle.js`.
 *
 * Same shape as `examples/anima-web/build.mjs` and for the same two reasons:
 ...
 *
 * The decoder takes its kernels as a *parameter* (`AudioKernels`), so no import
 * redirect is needed for them; the shim below exists only for `params()`, which
 * `decoder-gpu.ts` imports from the harness for its uniform packing. One small
