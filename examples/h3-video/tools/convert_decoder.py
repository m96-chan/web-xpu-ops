...
# **The latent space the DiT works in is not the one this decoder reads.**
# `AutoencoderKLMiniMaxH3`'s own doc: "a pipeline encodes with
# `(latent - latents_mean) / latents_std` and decodes with
# `latent * latents_std + latents_mean`". These are the per-channel statistics
# from the wrapper's `config.json`, carried in the manifest because the decoder
# is the thing that needs them and every caller was getting it wrong without
# them -- `examples/h3-dit`'s sampler decoded the DiT's output verbatim and the
# grid-textured mush that came back was read as int8 for weeks.
...
def main() -> int:
    ...
    manifest = {
        ...
        "latentsMean": list(LATENTS_MEAN), "latentsStd": list(LATENTS_STD),
