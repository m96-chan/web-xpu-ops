...
**A neural vocoder is five kernels.**

`conv`, `conv_transpose`, `pad`, `snake`, `axpy` — that is the entire decoder of
a BigVGAN-lineage codec, a 32-channel latent at 40 Hz in and 32 kHz audio out.
Four of the five had landed weeks earlier for an unrelated text-to-speech
project. Nothing new had to be written.

Thirty-two kernels are in here. Everything below is built from them, runs on
WebGPU, and is checked against the original model's own Python rather than
against another port.

![Anima-3.8B at 832x1216, generated by this library](examples/anima/fixtures/generated-1216x832.png)

<sub>Anima-3.8B, 832×1216, 40 steps, CFG 8, int8 weights — 233 s end to end on an
RTX 5090 through Dawn. Every number and the conditions it was taken under are in
[`examples/anima/`](examples/anima/).</sub>

---

## What has been built on them

| | in | out | ops | agreement with the original |
| --- | --- | --- | --- | --- |
| [**Anima-3.8B**](examples/anima/) | a prompt | an 832×1216 image | 11 | **1.18e-5** rel-RMS |
| [**Z-Image**](examples/zimage/) | a prompt | a 1024² image | 13 | 4.39e-6 |
| [**MiniMax-H3 R2V**](examples/h3-ref2v/) | reference images **and video** | video | 11 | held at every joint |
| [**MiniMax-H3's DiT**](examples/h3-dit/) | a prompt | a video latent | 8 | **1.49e-7** worst element |
| [**its visual VAE**](examples/h3-video/) | a latent | video frames | 9 | 4.53e-6 |
| [**its audio VAE**](examples/h3-audio-web/) | a 32-ch latent | 32 kHz audio | **5** | 5.01e-6 |
| [**Sarashina2.2-1B**](#llm-an-inference-engine-built-from-these-ops) | tokens | tokens | 10 | `transformers`, int8, GPU-resident decode |
| [**MioTTS**](https://github.com/m96-chan/voxshot) *(another repository)* | text | speech, cloned voice | — | its own goldens |

Twenty-one distinct ops cover all of it. Every example carries a section headed
**"No new kernel"** listing what it used; the last two ports — a fifty-block
video transformer and a reference-to-video pipeline — added none at all, and the
one before them changed a single binding type and wrote down why.

The audio decoder is the sharpest case. `conv_transpose1d` landed "so a codec
decoder can reach a waveform", `snake_beta` because that name covers two
different functions, `group_norm` for a codec's residual blocks — all three for
[VoxShot](https://github.com/m96-chan/voxshot)'s text-to-speech, before MiniMax
published a video model with native stereo audio. Its audio half needed exactly
those. The example's own note is shorter than this paragraph: *a DAC-lineage
codec is a DAC-lineage codec.*

## So how much of audio generation does this cover?

Every architecture below was read from its own source, not from a paper's prose.
The question is what each **decoder** needs at inference.

| architecture | what its decode path needs | status |
| --- | --- | --- |
| [**BigVGAN / v2**](https://arxiv.org/abs/2206.04658) | conv1d, conv_transpose1d, **depthwise** both, Snake / SnakeBeta, replicate pad | **all here** |
| [**DAC**](https://github.com/descriptinc/descript-audio-codec) | conv1d dilated, conv_transpose1d, Snake1d, tanh | **all here** |
| [**Vocos**](https://arxiv.org/abs/2306.00814) | depthwise conv1d, Linear, LayerNorm, GELU, **ISTFT** | **all here** |
| mel frontend | reflect pad, STFT, mel filterbank, log | **all here** |
| RVQ codebook decode | embedding gather, 1×1 conv, sum | **all here** |
| [**HiFi-GAN**](https://github.com/jik876/hifi-gan) | conv1d, conv_transpose1d, tanh, **LeakyReLU(0.1)** | one activation short |
| [**SNAC**](https://arxiv.org/abs/2410.14411) | DAC's set + windowed attention + **1-D `repeat_interleave`** | attention here, the repeat is not |
| [**EnCodec**](https://github.com/facebookresearch/encodec) | SEANet + ELU + reflect pad + a **2-layer LSTM** | the LSTM is not here |
| WaveNet / WaveRNN | one forward pass per output **sample** | a different machine, not a missing kernel |

BigVGAN's anti-aliasing filters look like they need a new op and do not: a
Kaiser-windowed sinc low-pass is a depthwise 1-D convolution with a fixed
kernel, and the kernel is generated once on the host. `ops/conv` and
`ops/conv_transpose` both take `groups`, so the filter is an argument rather
than an operator.

**Three gaps, and only one is interesting** (issue [#237](https://github.com/m96-chan/web-xpu-ops/issues/237)). `leaky_relu` is a missing case in
`ops/activation`. A 1-D `repeat_interleave` is a missing case in `ops/upsample`,
which today exports 2-D nearest only. Both are ordinary.

The LSTM is not. It is the only entry in that table that is a different *shape*
of computation rather than a different function — a sequential dependency across
frames, in a library whose entire premise is that work batches. It sits at the
front of EnCodec's decoder at the lowest frame rate it will ever see, so the
loop is short next to the audio it produces, but it is still a loop. Nothing
here has needed one yet, which is why nothing here has one.

## Why this is checked the way it is

Three rules, and they are what make the numbers above worth reading.

**The reference is the definition, and it is the slowest thing here.** Every op
has one: plain TypeScript, f64 accumulation, no GPU, written to be obviously
correct rather than quick. Kernels are compared to it and never to each other.
Backends multiply; correctness stays singular.

**A port is held to the model, never to another port.** Two implementations that
agree have only demonstrated that they share a mistake. So the audio decoder is
checked against a waveform `DacAudioVAE.decode` produced, unmodified, on a fixed
latent — not against the CPU port sitting beside it.

**A number without its conditions is not a measurement.** Device, driver,
backend, dtype, input shape, every time. Where a thing has not been measured the
table says *unmeasured*, rather than leaving a blank for a reader to fill in
optimistically.
...
ESM only, Node ≥ 20. Compiled JavaScript, `.d.ts` and the `.wgsl` files ship
together, so nothing at the other end needs a TypeScript toolchain.

```ts
import { matmul } from "web-xpu-ops/ops/matmul";                 // the reference
import code from "web-xpu-ops/ops/matmul/wgsl/kernel.wgsl?raw";  // the kernel
...
gets one. Whole models ship too — `web-xpu-ops/models/anima` and
`web-xpu-ops/llm/engine` — see [Models and engines](#models-and-engines).
...
[Z-Image](examples/zimage/) — MiniMax-H3's video, audio and reference-to-video
pipelines, and an LLM engine in `llm/`. The Anima port and the engine are
importable, as `web-xpu-ops/models/anima` and `web-xpu-ops/llm/engine` — see
"Models and engines" below. The audio ops have a consumer outside this
repository too: [**VoxShot**](https://github.com/m96-chan/voxshot) builds MioTTS
on them — browser text-to-speech and zero-shot voice cloning, `istft`'s `"same"`
padding, `snake.beta`, `group_norm` and `conv_transpose` among others. It is why
those ops have the conventions they do; the rows below name MioCodec because
MioCodec is what they were written against. There is no TTS example here,
deliberately — the port exists there and one copy of it is better than two.
