// FlashAttention-2. Generated by ops/flash_attention/tools/generate.ts — edit
...
fn main(
  ...
  for (var t = tid; t < BQ * d4n; t = t + THREADS) {
    ...
    if (row < params.L) {
      for (var c = 0u; c < 4u; c = c + 1u) {
        let d = c0 + c;
        if (d < params.D) { val[c] = q[((batch * params.L + row) * params.H + h) * params.D + d]; }
      }
    }
    sq[r * ROW + unit] = val;
  }
  ...
  for (var t = 0u; t < tiles; t = t + 1u) {
    ...
    for (var e = tid; e < TILE_S * d4n; e = e + THREADS) {
      ...
      if (j < params.S) {
        for (var c = 0u; c < 4u; c = c + 1u) {
          let d = c0 + c;
          if (d < params.D) { val[c] = k[((batch * params.S + j) * params.H + h) * params.D + d]; }
        }
      }
    ...
    for (var e = tid; e < TILE_S * params.Dv; e = e + THREADS) {
      ...
      let d = e % params.Dv;
      sv[(0u) * V_STRIDE + e] = select(0.0, v[((batch * params.S + j) * params.H + h) * params.Dv + d], j < params.S);
    }
    workgroupBarrier();
    ...
    for (var slot = 0u; slot < TILE_S; slot = slot + 1u) {
      ...
      for (var c = tid; c < params.Dv; c = c + THREADS) {
        let value = sv[(0u) * V_STRIDE + slot * params.Dv + c];
        for (var r = 0u; r < BQ; r = r + 1u) {
  ...
  for (var r = 0u; r < BQ; r = r + 1u) {
    ...
    var p = 0u;
    for (var c = tid; c < params.Dv; c = c + THREADS) {
      output[((batch * params.L + row) * params.H + h) * params.Dv + c] = select(acc[r][p] / denom, 0.0, denom == 0.0);
      p = p + 1u;
    }
  }
}
