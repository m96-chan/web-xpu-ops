...
function testFiles() {
  ...
  // llm/ is CPU-only (no GPU device involved at all — see llm/tokenizer.ts),
  // but it gets a process per file too for the same reason as the harness:
  // one discovery code path beats two that have to be kept in sync.
